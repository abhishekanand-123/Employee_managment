...
function AdminLeaveCalendar() {
    ...
    const fetchLeaves = async () => {
      try {
        ...
        const res = await axios.get(
          `http://localhost:5000/api/admin/leave/calendar?start=${start.toISOString()}&end=${end.toISOString()}`,
  ...
  return (
    <div>
      <h4 className="card-title mb-4">Leave Calendar</h4>
      <p className="card-description">Calendar-wise view of all employee leaves.</p>

      <div className="mb-3">
        <Link to="/leave-requests" className="btn btn-outline-primary btn-sm">
          View Leave Requests
        </Link>
      </div>
                      ...
                      <div className="calendar-day-leaves">
                        {dayLeaves.map((l) => (
                          <span
                            ...
                            title={`${getEmployeeName(l.employeeId)} - ${LEAVE_TYPE_LABEL[l.leaveType]} - ${l.status}`}
                          >
                            {getEmployeeName(l.employeeId)} ({LEAVE_TYPE_LABEL[l.leaveType]})
                          </span>
                        ))}
                      </div>
                    )}
